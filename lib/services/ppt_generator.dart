...
class PPTGenerator {
  static Future<File> generatePPT(List<Map<String, dynamic>> slides, String outputPath) async {
    try {
  ...
  static Uint8List _createPPTXArchive(List<Map<String, dynamic>> slides) {
...
<p:sldLayout xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" type="blank">
  <p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr/></p:spTree></p:cSld>
    ...
    for (int i = 0; i < slides.length; i++) {
      ...
      final slideXml = _buildSlideXml(slide, slideNum);
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  static String _buildSlideXml(Map<String, dynamic> slide, int slideNum) {
    final rawTitle = slide['title'] ?? 'Slide $slideNum';
    final rawHtml = slide['htmlContent'] ?? '';
    
    // Parse plain text content from HTML
    final document = html_parser.parse(rawHtml);
    final textContent = document.body?.text.replaceAll(RegExp(r'\s+'), ' ').trim() ?? rawHtml;

    final cleanTitle = _xmlEscape(rawTitle.toString());
    final cleanBody = _xmlEscape(textContent);
    ...
    <p:spTree>
      ...
      <p:sp>
        ...
        <p:spPr>
          ...
          <a:presetGeom geom="rect"><a:avLst/></a:presetGeom>
        </p:spPr>
        <p:txBody>
          <a:bodyPr/>
          <a:lstStyle/>
